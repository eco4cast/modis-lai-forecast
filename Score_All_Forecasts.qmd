...
for(i in subs_to_score){
  ...
  if(parametric_ind == 1){
    score_dir <- scoring_spat_parametric(file_dir, target, "score_dir")
    spat4cast_score_submit(score_dir, model_id, reference_date = ref_dates_to_score[i])
  }else{
    score_dir <- scoring_spat_ensemble(file_dir, target, "score_dir")
