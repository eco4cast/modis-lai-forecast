...
for(i in subs_to_score){
  ...
  files <- mc_cp(i, "files/")
  parametric_ind <- if_else(1 %in% str_detect(files, c("lognormal|normal|bernoulli|beta|uniform|gamma|logistic|exponential|poisson")) == TRUE, 1, 0)
  if(parametric_ind == 1){
    score_dir <- scoring_spat_parametric(i, target, "score_dir")
