...
sites <- list.files("shp/")

for(site in sites){
  
submitted <- mc_ls(paste0("efi/spat4cast-submissions/duration=P1M/variable=lai_recovery/site_id=", site,"/"), recursive = TRUE)
    
forecasts <- mc_ls(paste0("efi/spat4cast-forecasts/duration=P1M/variable=lai_recovery/site_id=" ,site,"/"), recursive = TRUE)
  
submitted <- unique(str_extract(submitted, ".*/reference_date=.*(?=/)"))

forecasts <- unique(str_extract(scored, ".*/reference_date=.*(?=/)"))

subs_to_process <- submitted[which(submitted %in% forecasts == 0)]

if(length(subs_to_process) > 0){
subs_to_process <- paste0("efi/spat4cast-submissions/duration=P1M/variable=lai_recovery/site_id=",site,"/",subs_to_process,"/")
  for(i in subs_to_process){
    ...
    file_names <- mc_ls(i)
    temp_rast <- terra::rast(file_names)
