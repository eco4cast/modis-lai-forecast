...
will be accepted up to 3 months ahead on a monthly time step.
...
target_dir <- spat4cast_get_target(date = "2025-03-01", fire = "august_complex")
...
To submit a forecast to the challenge, use the `spat4cast_submit` function. As arguments, this function requires a directory and a model_id for your model. The directory should contain all necessary GeoTiff files for your forecast (formatting requirements in next section). On the minIO bucket, forecasts with the same name are organized by the date that they are submitted. So, if you wish to submit multiple different forecasts on the same day, make sure your model_id is unique, or else it will be overwritten by subsequent submissions with the same model_id that are submitted on the same day. Notably, you should keep track of the day that you are submitting the forecast, since you will need a reference date to retrieve your score. 
...
ensemble_forecast_dir |>
  spat4cast_submit(model_id = 'johnsemble', site_id = "august_complex")

parametric_forecast_dir1 |>
  spat4cast_submit(model_id = 'parametsmith', site_id = "august_complex")
```
